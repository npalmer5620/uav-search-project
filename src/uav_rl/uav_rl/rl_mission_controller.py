...
from uav_rl.actions import ActionConfig, SearchAction, action_to_goal, move_delta, slew_yaw
...
class RLMissionController(MissionControllerBase):
    ...
    def _init_search_controller(self) -> None:
        self.rl_policy_version = str(self.get_parameter("rl.policy_version").value).strip() or "v2"
    ...
    def _search_step_v2(self) -> None:
        ...
        if need_decision:
            ...
            requested_goal_cell = self._v2_belief.geometry.world_to_grid(goal.x, goal.y)
            if (
                move_delta(requested_action) is not None
                and self._v2_previous_cell is not None
                and requested_goal_cell == self._v2_previous_cell
            ):
            ...
            if goal_cell != current_cell and move_delta(self._v2_last_action) is not None:
            ...
            if goal.target_xy is not None and best_track is not None:
